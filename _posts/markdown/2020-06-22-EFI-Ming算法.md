...
1. $E(\alpha)$：表示根据深度优先搜索遍历所有项，得到大于 $\alpha$ 的所有项集，有$E(\alpha) =$ {$ z \mid z \in I \land z \succ x, \forall x \in \alpha$}，如Fig.1，设 $\alpha = \{c\}$，则 $E(\alpha) = \{a, b, d\}$
...
+ 利用utility-bin计算$twu(X)$，有公式 $U[z] = U[z] + tu(T)$，其中 $z \in T$，在处理完最后一个事务时，对于 $\forall i \in I，twu(i) = U[i]$
  ...
  ![utility-bin]({{ "/assets/algorithm/EFI-Mining算法/计算效用箱数组.png" | absolute_url }})
